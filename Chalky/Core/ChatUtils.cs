...
public static class ChatUtils
{
    ...
    private static readonly Regex CommonTMPTagRegex = new Regex(
        @"<\/?(color(=#[0-9a-fA-F]{3,8})?|#[0-9a-fA-F]{3,8}|b|i|u|s|sup|sub|size(=[\d.]+%?)?|alpha=#[0-9a-fA-F]{2}|mark(=#[0-9a-fA-F]{3,8})?|uppercase|lowercase|smallcaps|nobr|noparse|sprite(=\d+)?|link(=[^>]*)?)>",
